...
sudo apt install -y mc zsh asciinema zram-config python3-pip htop sshpass p7zip-full ncdu apt-transport-https ca-certificates software-properties-common chkrootkit rkhunter okteta golang-go gccgo pinta gocryptfs audacity obs-studio cpu-checker iotop qemu-kvm bridge-utils gnome-games pwgen adb gpp remmina remmina-plugin-rdp remmina-plugin-vnc virtualbox-ext-pack vlc inkscape glogg gpick hardinfo virtualbox-qt sqlitebrowser pgmodeler umbrello kamoso bleachbit gparted build-essential git docker-compose fonts-liberation filelight gimp kdenlive qtqr transmission-qt pv handbrake flameshot laptop-mode-tools wireguard resolvconf
...
- Set ZSH default shell in Terminal App (like Konsole)
